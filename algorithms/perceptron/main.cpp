...
static vector<int> gridToInput(const Grid& grid) {
    vector<int> input(PERCEPTRON_INPUT_SIZE);
    for (int r = 0; r < GRID_SIZE; r++)
        for (int c = 0; c < GRID_SIZE; c++)
            input[r * GRID_SIZE + c] = grid[r][c] ? 1 : -1;
    input[PERCEPTRON_PIXELS] = 1;
    return input;
}
...
int main() {
    ...
    sf::RenderWindow window(
        ...
        "Perceptron Draw Pad  |  LMB: draw  |  Enter: classify  |  C: clear",
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed) {
                if (event.key.code == sf::Keyboard::Enter) {
                    ...
                    int cls = perceptron_classify(X, W);
                    cout << "Detected: " << perceptron_classLabel(cls)
                         << "  (class " << cls << ")" << endl;
                }
